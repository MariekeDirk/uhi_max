...
df<-try(df[complete.cases(df[[var]])],TRUE) # In my case I'm only interested in the temperature
try(write.table(df,paste0(folder.loc,"/",stationname,"/","daydata_",t,".txt"),row.names = FALSE,sep=","),TRUE)
